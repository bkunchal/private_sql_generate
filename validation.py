...
class LineLoader(yaml.SafeLoader):
    def construct_mapping(self, node, deep=False):
        ...
        mapping['__line__'] = node.start_mark.line + 1  # Track line number for this mapping
    ...
    def construct_sequence(self, node, deep=False):
        ...
        for idx, item in enumerate(node.value):
            if isinstance(item, yaml.ScalarNode):
                sequence[idx] = {
                    'value': item.value,
                    '__line__': item.start_mark.line + 1  # Capture line number for each list item
                }
        return sequence
...
def validate_string(value, field_name, line, logger):
    if not isinstance(value, str):
        ...
        return False
    if not value.strip():  # Ensure the string is non-empty after trimming
...
def validate_list(items, field_name, parent_line, logger):
    ...
    for idx, item in enumerate(items):
        line = parent_line  # Default to parent's line for individual strings
...
def validate_join_dict(join, field_name, line, logger):
    ...
    for field in required_fields:
        field_line = join.get(f"__{field}_line__", line)  # Track the line number for each field
